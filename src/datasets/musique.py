...
class MuSiQue(BaseDataset):
    ...
    def create_open_book_demo_text(self) -> List[str]:
        ...
        return demo_texts
        '''
        safe_len = min(len(questions), len(answers))
        
        if self.kwargs.get("use_chat_template", False):
            for i in range(safe_len):  # <--- 改用 safe_len
                demo_texts += [
                    f"Q: {questions[i]}\nA:",
                    answers[i],
                ]
    ...
    def build_prompt(self, contexts, question):
        # 1. 默认值
        instruction = ["Answer the following question."]
        var_str = str(self.variation).lower() if self.variation else ""

        # 2. CoT 判断
        if "cot" in var_str:
            instruction = ["Answer the following question by reasoning step-by-step."]

        # 3. Open/Closed 判断 (只靠字符串，不靠 n_docs 了)
        # 逻辑：如果 variation 包含 "closed"，或者明显不包含 "open"，就视作闭卷
        if "closed" in var_str:
            is_closed_book = True
        elif "open" in var_str:
            is_closed_book = False
        else:
            # 如果没写，默认是 Open Book (除非你显式指定了 closed)
            is_closed_book = False
        ...
        if self.kwargs.get("use_chat_template", False):
            ...
            prompted_question_wo_context = (
                instruction + demo_text + f"{verbalised_question}{answer_prefix}"
            )
